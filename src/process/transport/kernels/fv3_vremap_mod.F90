...
!***********************************************************************
!
! CATChem provenance note
! -----------------------
! This file is VENDORED VERBATIM (numerics unchanged) from the GFDL FV3
! dynamical core, model/fv_operators.F90:
!   https://github.com/NOAA-GFDL/GFDL_atmos_cubed_sphere  (branch: main)
!   downloaded 2026-07-09
! It remains under the GNU Lesser General Public License v3 (header above).
!
! Only the conservative 1-D vertical PPM remap routine `mappm` and its
! private helpers (cs_profile, cs_limiters, ppm_profile, ppm_limiters) are
! kept here; these are exactly the routines UFS-ATM (FV3) and GCHP use to
! remap tracers from the Lagrangian layers back to the reference grid.
!
! The ONLY changes from upstream are:
!   * module renamed  fv_operators_mod  ->  fv3_vremap_mod  (only the remap
!     routines are retained; horizontal/other routines are omitted)
!   * `call mpp_error(FATAL,'kord not implemented')` is replaced by
!     `error stop 'fv3_vremap_mod: kord not implemented'` to drop the FMS
!     (mpp) dependency; behaviour (abort on unsupported kord) is unchanged
! The bodies of mappm/cs_profile/cs_limiters/ppm_profile/ppm_limiters are
! IDENTICAL to upstream. When updating, re-download and re-apply only these
! header edits.
!
! Precision: compiled inside CATChem_process_transport with
! -fdefault-real-8 -fdefault-double-8, so bare `real` == REAL(8), matching
! the upstream FV3 default-real-8 build.
!***********************************************************************
