...
! dynamical core, model/tp_core.F90:
...
!
! The ONLY changes from upstream are in this header block:
!   * module renamed  tp_core_mod  ->  fv3_tp_core_mod
!   * the two FV3-framework `use` statements are replaced by CATChem's
!     trimmed `fv3_grid_types_mod` (same derived-type names, no FMS needed)
!   * the unused `big_number` import is dropped
! The body (fv_tp_2d, xppm, yppm, pert_ppm, copy_corners, deln_flux,
! mp_ghost_ew) is IDENTICAL to upstream. When updating, re-download and
! re-apply only these header edits.
!***********************************************************************

module fv3_tp_core_mod
!BOP
!
